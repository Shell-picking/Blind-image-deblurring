...
wide_stream output_data[2*PACKETS];
unsigned char output_buffer[W*H*2] = {0};
...
// 	canny_edge(input_data,output_data,imgSrc.rows,imgSrc.cols, th1, th2);
	...
	for (int i = 0; i < PACKETS*2; ++i) {
		...
		printf("%x %x %x %x\n",output_buffer[4*i],output_buffer[4*i+1],output_buffer[4*i+2],output_buffer[4*i+3]);
	}
